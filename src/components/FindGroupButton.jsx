...
function FindGroupButton() {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`http://192.168.180.164:8080/api/classes/${classId}`, {
        credentials: 'include'
      });
  ...
  const handleJoinClass = async () => {
    ...
    try {
      const response = await fetch(`http://192.168.180.164:8080/api/student-class/join/${classInfo.id}`, {
        method: 'POST',
        credentials: 'include',
        headers: {
